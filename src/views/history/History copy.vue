<template>
  <div class="container overflow-y-auto">
    ...
    <div v-if="loading" class="loading">Loading order history...</div>
    <div v-else-if="error" class="error">{{ error }}</div>
    <div v-else class="history-list overflow-auto">
      <button
...
export default {
  ...
  methods: {
    async fetchDashboardData() {
      ...
      this.error = null;
      try {
        const response = await HistoryService.listing('2023-01-01', '2023-12-31'); // Pass appropriate dates
        // this.historyItems = response.data; // Update based on API response structure
      } catch (error) {
